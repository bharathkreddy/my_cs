...
int main(void) {
    ...
    ret =
        connect(client_socket_fd, (const struct sockaddr *)&name, sizeof(name));
    if (ret == -1) error_handler("connect");
    printf("client socket connected.\n");

    /* write data */
    sprintf(buffer, "PID:%d", getpid());
    ret = write(client_socket_fd, buffer, strlen(buffer) + 1);
    if (ret == -1) error_handler("write");
    while (1) {
        memset(buffer, 0, BUFFER_SIZE);
        /* receive data */
        ret = read(client_socket_fd, buffer, BUFFER_SIZE);
        if (ret == -1) error_handler("read");
